...
$results = Caspsule::table('tblaccounts')
    ->select(
        'tblclients.id',
        'tblclients.firstname',
        'tblclients.lastname',
        Caspsule::raw('SUM(tblaccounts.amountin/tblaccounts.rate AS amountIn'),
    ...
    foreach($results as $result){
        ...
        // check if the client is from Greece
        $clientCountry = Caspsule::table('tblclients')
            ->where('id', $userid)
            ->pluck('country')
            ->first();
